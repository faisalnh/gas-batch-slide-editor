...
function updateLog(jobId, message, status) {
  ...
  const currentLog = cache.get(jobId) || JSON.stringify({ log: '', status: 'running' });
...
function getLogUpdates(jobId) {
  ...
  return logData ? JSON.parse(logData) : { status: 'error', log: 'Job ID not found or expired.' };
}
...
function deleteSlidesInFolder(jobId, formData) {
  try {
    ...
    if (!folderId || isNaN(startSlide) || isNaN(endSlide)) throw new Error("Invalid input.");
    ...
    while (files.hasNext()) {
      ...
      updateLog(jobId, `Processing file: ${file.getName()} (${totalSlides} slides).`);
...
function insertSlideFromSourceIntoAllPresentations(jobId, formData) {
  try {
    ...
    const destinationSlideIndex = parseInt(formData.insertDestPosition, 10) || 0;

    if (!sourcePresentationId || !destinationFolderId || isNaN(sourceSlideStartIndex) || isNaN(sourceSlideEndIndex)) throw new Error("Invalid input.");
    ...
    updateLog(jobId, `Source: ${sourcePresentation.getName()}, Destination Folder: ${folder.getName()}`);
    ...
    while (files.hasNext()) {
      ...
      if (destinationSlideIndex > 0) {
        ...
        for (let i = sourceSlideStartIndex - 1; i <= sourceSlideEndIndex - 1; i++) {
...
function replaceSlideInBatch(jobId, formData) {
  try {
    ...
    if (!replacedFolderId || !replacingFolderId || isNaN(replacedSlideNumber) || isNaN(replacingSlideNumber)) throw new Error("Invalid input.");
    ...
    const replacingFilesIterator = replacingFolder.getFilesByType(MimeType.GOOGLE_SLIDES);

    updateLog(jobId, `Building map from source folder: ${replacingFolder.getName()}`);
    while (replacingFilesIterator.hasNext()) {
      const file = replacingFilesIterator.next();
      replacingFilesMap.set(file.getName(), file.getId());
    }
    updateLog(jobId, `Found ${replacingFilesMap.size} source files. Starting replacements...`);
    
    const replacedFilesIterator = replacedFolder.getFilesByType(MimeType.GOOGLE_SLIDES);
    while (replacedFilesIterator.hasNext()) {
      ...
      if (replacingFilesMap.has(fileName)) {
        const destinationPresentation = SlidesApp.openById(replacedFile.getId());
        const sourcePresentation = SlidesApp.openById(replacingFilesMap.get(fileName));
        const oldSlide = destinationPresentation.getSlides()[replacedSlideNumber - 1];
        const newSlide = sourcePresentation.getSlides()[replacingSlideNumber - 1];
        
        destinationPresentation.insertSlide(replacedSlideNumber - 1, newSlide);
        oldSlide.remove();
        updateLog(jobId, `  - SUCCESS: Replaced slide in ${fileName}.`);
      } else {
        updateLog(jobId, `  - SKIPPED: No matching source file for ${fileName}.`);
      }
      Utilities.sleep(100);
    }
    updateLog(jobId, 'Process finished successfully.', 'completed');
